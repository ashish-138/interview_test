...
function createTable(data) {
  ...
  if (!data) {
    ...
    table.classList.remove("contain-table")
    table.classList.add("contain-table-1")
    table.innerHTML = `<h1>No data available</h1>`;
  } else {

    for(let i=0;i<data.length;i++){
        let invertData = 1/data[i].fields.rate;
        if(i%2==0){

            let tdata = `<tr class="table-body-1">
            <td>${data[i].datasetid}</td>
            <td>${data[i].fields.date}</td>
            <td>${data[i].fields.currency}</td>
            <td>${data[i].fields.rate.toFixed(4)}</td>
            <td>${invertData.toFixed(4)}</td>
            </tr>
            `
